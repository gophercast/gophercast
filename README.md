...
// Subscribe to topic (returns subscription)
sub := b.Subscribe(topic)
...
```go
// Create topic
topic, err := topic.New("topic-name")

// Get name
name := topic.String()

// Compare topics
equal := topic1.Equals(topic2)
```
...
```go
// Create message
msg := message.NewMessage(topic, data)

// Access properties
id := msg.ID()
topic := msg.Topic()
data := msg.Data()
time := msg.PublishedAt()
